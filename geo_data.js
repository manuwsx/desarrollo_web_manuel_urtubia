...
const geo_dropdowns = (selectRegionId, selectComunaId) => {
    const regionSelect = document.getElementById(selectRegionId); //obtenemos los select por su id
    const comunaSelect = document.getElementById(selectComunaId);

    if (!regionSelect || !comunaSelect) return;

    //dropdown de regiones
    for (const r in region_comuna) {
    ...
    regionSelect.addEventListener('change', () => {
        const region = regionSelect.value //region elegida
        const comunas = region_comuna[region] //comunas de la region elegida

        //cada vez que se cambie la region limpiamos las opciones
